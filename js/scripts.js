...
let pokemonRepository = (function () {
  ...
  searchBar.addEventListener("keyup", (e) => {
    ...
    let Pokemons = Array.from(
      document.querySelectorAll(".pokemon-list li")
    )

    let i
    for (i = 0; i < Pokemons.length; i++) {
      if (Pokemons[i].dataset.name.includes(searchString)) {
        Pokemons[i].removeAttribute("hidden")
      } else {
        Pokemons[i].setAttribute("hidden", true)
      }
    }
  })
